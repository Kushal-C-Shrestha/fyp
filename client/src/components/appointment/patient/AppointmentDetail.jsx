...
import { X, Clock4, Video, Loader2, FileText, Paperclip } from 'lucide-react';
...
const isVideoAppointment = (apt) => {
  const t = String(apt?.appointment_type || apt?.mode || '').toLowerCase();
...
const AppointmentDetail = ({ selected, setSelectedId, setRescheduleTarget, handleCancel, cancelLoading, callStatus = 'waiting', onRecordsAttached }) => {
  ...
  const isVideo = isVideoAppointment(selected);
    ...
    <section className="min-w-0 flex-1 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isVideo && (
            callEnded ? (
              ...
              <button
                type="button"
                onClick={() => openPatientVideoCall(selected)}
          ...
          <button
            type="button"
            onClick={() => handleCancel(selected.appointment_id)}
      ...
      <div className="space-y-3 text-sm text-slate-700">
        <div className="grid gap-3 sm:grid-cols-2">
          <p><span className="font-semibold text-slate-900">Date:</span> {formatShortDate(selected.appointment_date)}</p>
          <p><span className="font-semibold text-slate-900">Time:</span> {formatTime(selected.appointment_time, '-')}</p>
        </div>
        <p><span className="font-semibold text-slate-900">Doctor:</span> {selected.doctor_name || '-'}</p>
        <p><span className="font-semibold text-slate-900">Hospital:</span> {selected.hospital_name || '-'}</p>
        <p><span className="font-semibold text-slate-900">Specialization:</span> {selected.specialization_name || 'General Medicine'}</p>
        <p><span className="font-semibold text-slate-900">Reason:</span> {selected.appointment_reason || '-'}</p>
        <p><span className="font-semibold text-slate-900">Type:</span> <span className="capitalize">{selected.appointment_type || '-'}</span></p>
        {selected.doctor_notes && (
          <p><span className="font-semibold text-slate-900">Doctor&apos;s Notes:</span> {selected.doctor_notes}</p>
        )}
        <div>
          ...
          {selected.attached_records?.length > 0 ? (
            ...
            <p className="mt-1 text-slate-500">No records attached.</p>
          )}
          {recordError && <p className="mt-2 text-xs font-semibold text-rose-600">{recordError}</p>}
        </div>

        {selected.appointment_status === 'scheduled' && attachOpen && (
      ...
      {isVideo && (
        <div className="mt-8 border-t border-slate-200 pt-6">
          <AppointmentChat
            ...
            participantName={selected.doctor_name}
            participantFallback="Doctor"
            canSend={selected.appointment_status === 'scheduled'}
          />
        </div>
      )}
    </section>
