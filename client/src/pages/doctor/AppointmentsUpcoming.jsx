...
const AppointmentsUpcoming = () => {
  ...
  }, [upcomingAppointments, activeTab]);

  const summary = useMemo(() => {
    return {
      pending: upcomingAppointments.filter((item) => {
        const status = normalizeStatus(item.appointment_status);
        return status === "scheduled" || status === "pending";
      }).length,
      completed: upcomingAppointments.filter((item) => normalizeStatus(item.appointment_status) === "completed").length,
      cancelled: upcomingAppointments.filter((item) => normalizeStatus(item.appointment_status) === "cancelled").length,
    };
