...
import { Building2, ChevronLeft, ChevronDown, Star, MapPin } from 'lucide-react';
...
const HospitalReviews = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const calls = [api.get(`/hospitals/${id}/reviews`)];
        if (!hospital) calls.unshift(api.get(`/hospitals/${id}`));
        
        const results = await Promise.all(calls);
        if (!hospital) {
          setHospital(results[0]?.data?.hospital || null);
        }
        
        const reviewsRes = hospital ? results[0] : results[1];
  ...
  const sorted = [...reviews].sort((a, b) => {
    ...
    return new Date(b.created_at || b.review_date) - new Date(a.created_at || a.review_date);
  });
  ...
  if (!hospital) {
    return (
      <div className="min-h-screen bg-white">
        <Navbar />
        <main className="mt-[74px] px-4 py-8 sm:px-6 lg:px-10 text-center">
          <p className="text-sm text-slate-500">Hospital not found.</p>
          <button onClick={() => navigate('/hospitals')} className="mt-4 px-4 py-2 bg-slate-900 text-white rounded-xl text-sm font-semibold hover:bg-slate-800 transition">
            Back to Hospitals
          </button>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="mt-[74px] px-4 py-7 sm:px-6 lg:px-10">
        ...
        <button onClick={() => navigate(`/hospitals/${id}`)}
          className="mb-5 inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-emerald-700 transition">
          <ChevronLeft className="h-4 w-4" /> Back to hospital details
        </button>

        {/* Header */}
        <section className="border-b border-slate-200 pb-6">
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 flex items-center justify-center">
              {hospital.hospital_image ? (
                <img src={hospital.hospital_image} alt={hospital.hospital_name} className="h-full w-full object-cover" />
              ) : (
            ...
            <div>
              <p className="text-xs font-semibold tracking-widest text-emerald-700 uppercase">{hospital.hospital_type || 'Private Hospital'}</p>
              <h1 className="mt-0.5 text-xl font-bold text-slate-900 sm:text-2xl">{hospital.hospital_name}</h1>
              <p className="mt-1 flex items-center gap-1.5 text-sm text-slate-500">
                <MapPin className="h-4 w-4 shrink-0 text-slate-400" />{hospital.hospital_address || 'Address unavailable'}
              </p>
              {total > 0 && (
                ...
                <div className="space-y-5">
                  {paged.map(review => {
                    ...
                    const createdAt = review.created_at || review.review_date
                      ? new Date(review.created_at || review.review_date).toLocaleDateString([], { year: 'numeric', month: 'short', day: 'numeric' })
                      : '-';
                    return (
                      <article key={review.review_id} className="border-b border-slate-100 pb-5 last:border-b-0 last:pb-0">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex min-w-0 items-center gap-3">
                            <UserAvatar src={review.reviewer_profile || review.reviewer_picture} name={review.reviewer_name} size="h-10 w-10" />
